;=============================================================================
; SENTINEL Black Jack (bj.asm) [last modified: 2026-05-04]
...
_bj8:		inc (hl)		; card drawn
		...
		push af
		call _hex2dec		; display hand total
		pop af
		cp 16h			; > 21 ?
		jr c, _bj9
		ld b, a			; bust
...
_bj12:		jr nc, _bj13
		...
		ld iyh, d
_bj13:		ld a, ixh
		call _hex2dec		; display winner score
		ld (hl), 24h		; ' '
		inc hl
		ld (hl), 38h		; '>'
		inc hl
		...
		inc hl
		ld a, iyh
		call _hex2dec		; display loser score
		ld (hl), 24h		; ' '
		inc hl
		ld (hl), b		; display '1' | '2'
		inc hl
...
_bj10:		ld ixh, 00h		; bust -> hand total = 0
		ld de, 0086h
		ld hl, _datbjb		; display "YOU BUST!"
_bjX:		ld a, (hl)
		inc hl
		cp ffh
		jr z, _bj11
...
_datbjb:
; "YOUBUST!"
db 22h, 18h, 1eh, 0bh, 1eh, 1ch, 1dh, 25h, 24h, ffh
;-----------------------------------------------------------------------------
; CHANGES:
; - R1: added "YOU BUST!" anim, code cleanup
;=============================================================================
